...
Community_longForm_2024 <- read_csv("Community_longForm_2024.csv")
...
# 2024
Unburned2024 <- as.data.frame(nulling_me_softly(Community_longForm_2024, traits=traitmatrix, plots=1:20, Nsim=999)) %>%
  mutate(year=5, severity="U") %>%
  rownames_to_column(var="sim")
Low2024 <- as.data.frame(nulling_me_softly(Community_longForm_2024, traits=traitmatrix, plots=21:40, Nsim=999)) %>%
  mutate(year=5, severity="L") %>%
  rownames_to_column(var="sim")
High2024 <- as.data.frame(nulling_me_softly(Community_longForm_2024, traits=traitmatrix, plots=41:60, Nsim=999)) %>%
  mutate(year=5, severity="H") %>%
  rownames_to_column(var="sim")
null2024 <- rbind(Unburned2024, Low2024, High2024)
write.csv(null2024, "Null Model/nullBeta2024.csv", row.names=FALSE)

# Bind them
null2020 <- read_csv("Null Model/nullBeta2020.csv")
# null2021 <- read_csv("Null Model/nullBeta2021.csv")
# null2022 <- read_csv("Null Model/nullBeta2022.csv")
# null2023 <- read_csv("Null Model/nullBeta2023.csv")
# null2024 <- read_csv("Null Model/nullBeta2024.csv")

nullBeta <- rbind(null2020, null2021, null2022, null2023, null2024)
